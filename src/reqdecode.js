...
function DecodePaymentRequest(req) {
    if (!(typeof req === 'string' || req instanceof String)) {
        throw Error("payment request should be a string")
    }
    if (!req) {
        throw Error("payment request should not be empty")
    }
    if (req.length<totalExpectedLen) {
        throw Error("payment request is too short, length="+req.length()+", need at least "+totalExpectedLen)
    }
