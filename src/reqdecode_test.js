...
for(let i=0; i<decodePaymentTestCases.length; i++) {
    decodedReq = DecodePaymentRequest(decodePaymentTestCases[i].reqStr)
    expectedReq = decodePaymentTestCases[i].req
    if (decodedReq.amount !== expectedReq.amount) {
        throw Error("Amount does not much " + decodedReq.amount + " !=" + expectedReq.amount)
    }
    if (!isEqual(decodedReq.destination.buffer, expectedReq.destination.buffer)) {
        throw Error("Destination does not much")
    }
    if (!isEqual(decodedReq.paymentHash.buffer, expectedReq.paymentHash.buffer)) {
        throw Error("Payment hash does not much")
    }
}